...
def get_lrpwrapperformodule(module, lrp_params, lrp_layer2method, thisis_inputconv_andiwant_zbeta=False):
  ...
  if isinstance(module, nn.ReLU):
    ...
    if type(autogradfunction) == tensorbiasedconv2d_beta0_wrapper_fct:
      ...
      print('unknown autogradfunction', type(autogradfunction) )
      exit()
  elif (module.__name__=='MemoryEfficientSwish') or (module.__name__=='Swish'):
    #return zeroparam_wrapper_class( module , relu_wrapper_fct() )
...
def convafterbn_returntensorbiasedconv(conv,bn): #after visatt
    ...
    #print('devices', conv.weight.device, w_bn.device, var_bn.device )
...
class clamplayer(nn.Module):
  ...
  def forward(self,x):
    ...
    self.w_bn_sign= self.w_bn_sign.to(x.device)
...
class  tensorbiased_linearlayer(nn.Module):
  ...
  def forward(self,x):
    y= self.linearbase.forward(x) + self.biastensor #.to(x.device)
    return y
...
class  tensorbiased_convlayer(nn.Module):
  ...
  def gettensorbias(self,x):
    ...
    self.inputfornewbias = self.inputfornewbias.to(x.device)
    with torch.no_grad():
...
class invertedposnegconv_tensorbiased(nn.Module):
    ...
    def __init__(self, tensorbiasedconv, ignorebias):
      super(invertedposnegconv_tensorbiased, self).__init__()
...
class tensorbiasedconv2d_betaany_wrapper_fct(torch.autograd.Function):
    """
    We can implement our own custom autograd Functions by subclassing
    torch.autograd.Function and implementing the forward and backward passes
    which operate on Tensors.
    """
    @staticmethod
    def forward(ctx, x, tensorbiasedclass, lrpignorebias, beta):
        """
        In the forward pass we receive a Tensor containing the input and return
        a Tensor containing the output. ctx is a context object that can be used
        to stash information for backward computation. You can cache arbitrary
        ...
        lrpignorebiastensor=torch.tensor([lrpignorebias], dtype=torch.bool, device= tensorbiasedclass.baseconv.weight.device)
        ctx.save_for_backward(x, tensorbiasedclass.baseconv.weight.data.clone(), bias, tensorbiasedclass.conv.weight.data.clone(), inputfornewbias,lrpignorebiastensor, beta, *values ) # *values unpacks the list
    ...
    def backward(ctx, grad_output):
        ...
        input_, conv2dweight, conv2dbias, newweight, inputfornewbias,lrpignorebiastensor, beta, *values  = ctx.saved_tensors
        print('retrieved', len(values))
        #######################################################################
        # reconstruct dictionary of config parameters
        def tensorlist_todict(values): 
        ...
        tensorbiasedclass=   tensorbiased_convlayer ( newweight = newweight , baseconv = conv2dclass , inputfornewbias = inputfornewbias )
        pnconv = posnegconv_tensorbiased(tensorbiasedclass, ignorebias= lrpignorebiastensor.item() )
        invertedpnconv = invertedposnegconv_tensorbiased(tensorbiasedclass, ignorebias= lrpignorebiastensor.item() )
...
class tensorbiasedconv2d_betaadaptive_wrapper_fct(torch.autograd.Function):
    """
    We can implement our own custom autograd Functions by subclassing
    torch.autograd.Function and implementing the forward and backward passes
    which operate on Tensors.
    """
    @staticmethod
    def forward(ctx, x, tensorbiasedclass, lrpignorebias, maxbeta):
        """
        In the forward pass we receive a Tensor containing the input and return
        a Tensor containing the output. ctx is a context object that can be used
        to stash information for backward computation. You can cache arbitrary
        ...
        lrpignorebiastensor=torch.tensor([lrpignorebias], dtype=torch.bool, device= tensorbiasedclass.baseconv.weight.device)
        ctx.save_for_backward(x, tensorbiasedclass.baseconv.weight.data.clone(), bias, tensorbiasedclass.conv.weight.data.clone(), inputfornewbias,lrpignorebiastensor, maxbeta, *values ) # *values unpacks the list
    ...
    def backward(ctx, grad_output):
        ...
        input_, conv2dweight, conv2dbias, newweight, inputfornewbias,lrpignorebiastensor, maxbeta, *values  = ctx.saved_tensors
        print('retrieved', len(values))
        #######################################################################
        # reconstruct dictionary of config parameters
        def tensorlist_todict(values): 
