...
def get_wrapped_resnet50_imagenet(weightpath, key, device):
  """
  Get Wrapped ResNet50 model loaded into the device. Written by Prof. Alex.
...
def get_lrp_explanations_for_batch(model, 
  ...
  with torch.enable_grad():
        if minus_fx:
            z = torch.sum( -outputs[:, predclasses] ) # Explain -f(x) if images are real
      
        else:
            z = torch.sum( outputs[:, predclasses] ) # Explain f(x) if images are fake
...
def main():
  ...
  weightfn = './weights/blur_jpg_prob0.5.pth'
  ...
  gan_and_classes['stylegan2'] = ['church', 'cat', 'car']
  #gan_and_classes['cyclegan'] = ['horse']
  #gan_and_classes['stylegan'] = ['car']
  #gan_and_classes['cyclegan'] = ['apple', 'orange']
  ...
  for gan_name in gan_and_classes:
    for clss in gan_and_classes[gan_name]:
      ...
      #clss='sr'
      outpath = './hms/imagenet_lrp_heatmaps/{}/{}/'.format(gan_name, clss)
      save_pt_files = False # No need to save .pt files.
      num_instances_real, num_instances_fake = 0, 500 # Use 1000 real and fake samples for analysis
      ## ------------End of Parameters--------


      # Model
      model_e = get_wrapped_resnet50_imagenet(weightfn, key, device)
