...
def get_wrapped_resnet50(weightpath, key, device):
  """
  Get Wrapped ResNet50 model loaded into the device.
  
  Args:
    weightspath : path of universal detector classifier weights
    key         : LRP key
  ...
  if key == 'beta0':
      ...
      'nn.ReLU':          relu_wrapper_fct,
      'nn.BatchNorm2d':   relu_wrapper_fct,
      'nn.Conv2d':        conv2d_betaany_wrapper_fct,
      'nn.Linear':        linearlayer_eps_wrapper_fct,  
      'nn.AdaptiveAvgPool2d': adaptiveavgpool2d_wrapper_fct,
      'nn.MaxPool2d': maxpool2d_wrapper_fct,
      'sum_stacked2': eltwisesum_stacked2_eps_wrapper_fct,
      ...
      lrp_layer2method={
          'nn.ReLU':          relu_wrapper_fct,
          'nn.BatchNorm2d':   relu_wrapper_fct,
          'nn.Conv2d':        conv2d_betaadaptive_wrapper_fct,
          'nn.Linear':        linearlayer_eps_wrapper_fct,  
          'nn.AdaptiveAvgPool2d': adaptiveavgpool2d_wrapper_fct,
          'nn.MaxPool2d': maxpool2d_wrapper_fct,
          'sum_stacked2': eltwisesum_stacked2_eps_wrapper_fct,
  ...
  model_e.copyfromresnet(model0, lrp_params = lrp_params_def1, lrp_layer2method = lrp_layer2method )
...
def normalize_relevances(lrp_explanations, only_positive=False, eps=1e-6):
  ...
  Scheme : Use absolute relevances for normalization
  Args:
...
def rank_fmaps(parent_dir, gan_name, 
  ...
  outpath_channelwise = os.path.join('./fmap_relevances/', 'resnet50', '{}_{}'.format(gan_name, aug)) 
  
  #save_pt_files_and_lrp = False # save .pt files and lrp visualizations if required.
  outpath = './output/resnet50/{}_{}/'.format(gan_name, aug) # For saving .pt files if required

  # Model
  blockPrint()
  model_e = get_wrapped_resnet50(weightfn, key, device)
  enablePrint()
  ...
  # Do note you might expect very small numerical changes when you repeat this experiment due to random resized crop
  # But the resulting ranking and topk feature maps should be very similar
