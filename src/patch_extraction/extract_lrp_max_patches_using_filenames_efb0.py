...
class EfficientNet_canonized_modfwd(EfficientNet_canonized):
    ...
    def __init__(self, blocks_args=None, global_params=None):
        ...
        super(EfficientNet_canonized_modfwd, self).__init__(blocks_args=blocks_args, global_params=global_params)
...
def get_high_activation_patches(feature_map_name, arch, gan_name, aug, bsize, num_instances=None):
    ...
    # somedict = torch.load(weightfn)
    # model0.load_state_dict( somedict['model']   )
    # model0.eval()

    blockPrint()
    model_e = EfficientNet_canonized_modfwd.from_pretrained('efficientnet-b0', num_classes=1, image_size=None, dropout_rate= 0.0 , drop_connect_rate=0.0)
    model_e.copyfromefficientnet( model, lrp_params_def1, lrp_layer2method)
    model_e.to(device)
    enablePrint()
